...
export default function HomePage({ onAuthed }) {
  ...
  // We no longer proxy a synthetic click to a hidden GIS button — that is
  // unreliable across browsers (it silently did nothing on Windows/Chrome).
  // Instead we render the REAL GIS button as a transparent (opacity-0) overlay
  // ON TOP of our styled button, so the user's physical click lands directly on
  // Google's button (a genuine user gesture) while the visible layer stays a
  // generic "Continue with Google" with no personalized-name flicker.
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col items-center px-6 pb-20 pt-16 text-center">
          ...
          <div className="mt-8 flex flex-col items-center gap-3">
            {configured ? (
              <div className="relative h-[44px] w-[300px]" aria-busy={busy}>
                {/* Visible, application-owned presentation. It must NOT receive
                    the click (pointer-events-none) — the real GIS button above
                    it does. Always shows generic text, so no name flicker. */}
                <button
                  type="button"
                  aria-hidden="true"
                  disabled={!ready || busy}
                  className="pointer-events-none absolute inset-0 flex w-full items-center justify-center gap-3 rounded-full bg-white px-6 text-[15px] font-medium text-slate-700 shadow-lg transition disabled:opacity-60"
                >
                  {busy ? (
                    <><Loader2 className="h-5 w-5 animate-spin text-slate-500" /> Signing you in…</>
                  ) : (
                    <>
                ...
                {/* Real Google button — transparent, in-layout, on top. It owns
                    the physical click and drives the ID-token flow. */}
                <div
                  ref={gsiRef}
                  data-ready={ready}
