...
SAFETY (deployment): this is a MANUAL developer/test seed only. It performs a
destructive delete_many() on the demo pid+asset, so ALL executable work is guarded
behind `if __name__ == '__main__'` AND an explicit `--force` flag. It is never imported
or run automatically by the app, deploy pipeline, or any scheduler. It only ever touches
the scoped demo PID + JRNY test asset — never real user data.
"""
...
def seed():
    """Destructive: wipes + reseeds the JRNY demo journey for the demo PID only."""
    clean()
...
if __name__ == '__main__':
    ...
    if '--force' not in sys.argv:
        print('REFUSING TO RUN: this DELETES the JRNY demo data for the demo PID.')
        print('This manual dev-only seed must never run automatically. Re-run with --force to confirm.')
        raise SystemExit(1)
    seed()
